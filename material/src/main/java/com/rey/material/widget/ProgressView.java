...
public class ProgressView extends View implements ThemeManager.OnThemeChangedListener {
    ...
    public void setProgressMode(int mode) {
        if (mCircular)
            ((CircularProgressDrawable) mProgressDrawable).setProgressMode(mode);
        else
            ((LinearProgressDrawable) mProgressDrawable).setProgressMode(mode);
    }
     ...
     */
    public void start() {
        if (mProgressDrawable != null && getProgressMode() == MODE_INDETERMINATE) {
